...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        dout1, grads['W2'], grads['b2'] = affine_backward(dout, cache2)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3 * 32 * 32, num_classes=10,
        ...
        for i, hd in enumerate(hidden_dims):
          self.params['W%d'%(i + 1)] = weight_scale * np.random.randn(next_input_dim, hd)
        ...
        self.params['W%d'%(self.num_layers)] = weight_scale * np.random.randn(next_input_dim, num_classes)
    ...
    def loss(self, X, y=None):
        ...
        # layer, etc.                                                              #
        ############################################################################
        scores, caches, L2_reg, dropout_caches = X, {}, 0.0, {}
        for i in range(self.num_layers - 1):
          if self.use_batchnorm:
            scores, caches[i] = affine_bn_relu_forward(scores, self.params['W%d'%(i + 1)], 
                                self.params['b%d'%(i + 1)], self.params['gamma%d'%(i + 1)], 
                                self.params['beta%d'%(i + 1)], self.bn_params[i])
          else:
            scores, caches[i] = affine_relu_forward(scores, self.params['W%d'%(i + 1)], self.params['b%d'%(i + 1)])
          if self.use_dropout:
            scores, dropout_caches[i] = dropout_forward(scores, self.dropout_param)
          L2_reg += np.sum(self.params['W%d'%(i + 1)]**2)

        scores, caches[self.num_layers - 1] = affine_forward(scores, self.params['W%d'%(self.num_layers)], self.params['b%d'%(self.num_layers)])
        L2_reg += np.sum(self.params['W%d'%(self.num_layers)]**2)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        dout, grads['W%d'%(self.num_layers)], grads['b%d'%(self.num_layers)] = affine_backward(dout, caches[self.num_layers-1])
